...
<form action="{{ route($route, $model) }}" method="post" class="bg-gray-100 flex flex-col p-8 rounded-2xl w-3/4 justify-center mx-auto mt-8 mb-6">
    ...
    <input class="bg-white border border-gray-300 text-gray-900 text-l rounded-lg p-2 w-2/5 @error('name') is-invalid @enderror" name="name" type="text" value="{{ old('name', $model->name) }}">
    </div>
